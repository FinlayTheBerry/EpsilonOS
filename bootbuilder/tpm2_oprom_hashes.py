...
def ParseUEFIDevPath(dev_path):
	UEFI_PATH_TYPES = {
		...
		0x03: {
			...
			17: "Logical Unit",
			15: "USB Class",
			6: "I2O Random Block Storage Class",
			11: "MAC Address",
			12: "IPv4 Address",
			13: "IPv6 Address",
			20: "Vlan 802.1q",
		...
		0x04: {
			...
			7: "PIWG Firmware Volume",
			8: "Relative Offset Range",
			9: "RAM Disk Device Path"
			},
		0x05: {
			"base": "BIOS Boot Spec",
			1: "BIOS Boot Spec v1.01"
			},
		0x7F: {
			"base": "End",
			0xFF: "End All",
	...
	nodes = []
	offset = 0
	while offset < len(dev_path):
		type, subtype, length = struct.unpack_from("<BBH", dev_path, offset)
		type_str = UEFI_PATH_TYPES[type]["base"] if type in UEFI_PATH_TYPES else f"Unknown 0x{type.to_bytes(1).hex()}"
		subtype_str = UEFI_PATH_TYPES[type][subtype] if type in UEFI_PATH_TYPES and subtype in UEFI_PATH_TYPES[type] else f"Unknown 0x{subtype.to_bytes(1).hex()}"
		nodes.append(f"\"{type_str}\".\"{subtype_str}\"")
		if type == 0x7F and subtype == 0xFF:
			break
		offset += length
	return " -> ".join(nodes)
...
def Main():
	...
	with open ("/sys/kernel/security/tpm0/binary_bios_measurements", "rb") as fp:
		buffer = fp.read()
		evlog = EventLog(buffer, len(buffer))
		for event in list(evlog):
			if event.evtype == EV_EFI_BOOT_SERVICES_DRIVER:
				print(f"{ParseUEFIDevPath(bytes.fromhex(event.devpath))} -> {event.digests[ALGORITHM_ID_SHA256].digest.hex()}")
	return 0
sys.exit(Main())